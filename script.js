function todayISO() {
    ...
    // Any record/sale for a sectioned plantation that predates the section field,
    // or was left blank, is treated as belonging to that plantation's first section.
    ...
    function getSaleSection(s) {
      const sections = getSectionsFor(s.plantation_type);
      if (!sections) return null;
      return s.section || sections[0];
    }

    function computeSectionStats(type, section) {
      const secRecords = records.filter(r => r.plantation_type === type && getRecordSection(r) === section);
      const laborCost = round2(secRecords.reduce((s, r) => s + r.labor_cost, 0));
      const itemsCost = round2(secRecords.reduce((s, r) => s + r.items_total, 0));
      const expenditure = round2(laborCost + itemsCost);
      const yieldKg = round2(secRecords.reduce((s, r) => s + (r.yield_kg || 0), 0));
      const workerCount = secRecords.reduce((s, r) => s + r.workers.length, 0);
      const itemCount = secRecords.reduce((s, r) => s + r.items.length, 0);
      const secSales = plantationSales.filter(s => s.plantation_type === type && getSaleSection(s) === section);
      const salesTotal = round2(secSales.reduce((s, x) => s + x.total, 0));
      const salesQty = round2(secSales.reduce((s, x) => s + x.quantity, 0));
      const net = round2(salesTotal - expenditure);
      return {
        type, section,
        recordCount: secRecords.length, laborCost, itemsCost, expenditure, yieldKg, workerCount, itemCount,
        saleCount: secSales.length, salesQty, salesTotal, net
      };
    }
    ...
    function netSalesFor(type) {
      return round2(effectiveGrossSalesFor(type) - expenditureFor(type));
    }

    function computeSalesTotalsFor(type) {
      const sales = plantationSales.filter(s => s.plantation_type === type);
      return {
        count: sales.length,
        quantity: round2(sales.reduce((s, x) => s + x.quantity, 0)),
        total: round2(sales.reduce((s, x) => s + x.total, 0))
      };
    }

    // Gross Sales used everywhere for Net Sales / Harvest Shares = whatever's manually
    // entered in Reports (for revenue not tied to a logged customer) PLUS every customer
    // sale logged on that plantation's page — combined automatically, no manual step needed.
    function effectiveGrossSalesFor(type) {
      return round2((grossSales[type] || 0) + computeSalesTotalsFor(type).total);
    }
    ...
    function updateRecordSectionField() {
      const type = document.getElementById('f-type').value;
      const sections = getSectionsFor(type);
      const field = document.getElementById('f-section-field');
      const select = document.getElementById('f-section');
      if (sections) {
        select.innerHTML = sections.map(s => `<option>${s}</option>`).join('');
        field.style.display = '';
      } else {
        select.innerHTML = '';
    ...
    function updateSaleSectionField() {
      const type = selectedPlantationType;
      const sections = getSectionsFor(type);
      const field = document.getElementById('sale-section-field');
      const select = document.getElementById('sale-section');
    ...
    async function saveRecord() {
      ...
      const type = document.getElementById('f-type').value;
      const sections = getSectionsFor(type);
      const section = sections ? (document.getElementById('f-section').value || sections[0]) : null;
    ...
    async function saveActivity() {
      ...
      try {
        if (editingActivityId) {
          ...
          if (idx !== -1) {
            activityRecords[idx] = { ...activityRecords[idx], date, description, plantation_type: plantation, image: activityImageData };
          }
        ...
        console.error(e);
        showToast('⚠️ Failed to save diary entry — the photo may be too large. Try a smaller image.', 'error');
      }
      setLoading(false);
    }
    ...
    function editActivity(id) {
      ...
      if (a.image) {
        activityImageData = a.image;
        showActivityImagePreview(a.image);
      } else {
        removeActivityImage();
      }
    ...
    // ---------- Journal photo handling (upload, paste, drag & drop) ----------
    function resizeAndReadImage(file, maxDim = 1600, quality = 0.82) {
      return new Promise((resolve, reject) => {
    ...
    async function handleActivityImageFile(file) {
      if (!file) return;
      try {
        showToast('📷 Processing photo…', 'info', 1500);
        const dataUrl = await resizeAndReadImage(file);
        activityImageData = dataUrl;
        showActivityImagePreview(dataUrl);
      } catch (e) {
        console.error(e);
        showToast('⚠️ Could not use that file — please choose an image', 'error');
      }
    }

    function showActivityImagePreview(dataUrl) {
      const img = document.getElementById('activity-image-preview');
      const empty = document.getElementById('activity-image-empty');
      img.src = dataUrl;
      img.style.display = 'block';
      empty.style.display = 'none';
      document.getElementById('activity-image-remove-row').style.display = '';
    }

    function removeActivityImage() {
      activityImageData = null;
      const img = document.getElementById('activity-image-preview');
      const empty = document.getElementById('activity-image-empty');
      ...
      const removeRow = document.getElementById('activity-image-remove-row');
      if (removeRow) removeRow.style.display = 'none';
    }

    function setupActivityImageHandlers() {
      const dropzone = document.getElementById('activity-image-dropzone');
      const input = document.getElementById('activity-image-input');
      if (!dropzone || !input) return;

      dropzone.addEventListener('click', (e) => {
        if (e.target.closest('#activity-image-remove-row')) return;
        input.click();
      });
      ...
      input.addEventListener('change', () => {
        if (input.files && input.files[0]) handleActivityImageFile(input.files[0]);
      });
      ...
      dropzone.addEventListener('drop', (e) => {
        ...
        if (file) handleActivityImageFile(file);
      });

      // Paste an image (Ctrl+V) anywhere while the Daily Journal tab is open
      document.addEventListener('paste', (e) => {
        const activeView = document.querySelector('.view.active')?.id;
        if (activeView !== 'view-activities') return;
        const items = e.clipboardData?.items;
        if (!items) return;
        for (const item of items) {
          if (item.type && item.type.startsWith('image/')) {
            const file = item.getAsFile();
            if (file) { handleActivityImageFile(file); e.preventDefault(); }
            break;
          }
        }
      });
    ...
    function renderActivities() {
      ...
      out.innerHTML = list.map(a => `
        <div style="margin-bottom:22px;padding-bottom:18px;border-bottom:1px dashed var(--border);">
          ...
          ${a.image ? `<img src="${a.image}" alt="Journal photo for ${a.date}" style="max-width:100%;max-height:360px;border-radius:8px;display:block;margin:0 0 12px;cursor:zoom-in;" onclick="window.open(this.src, '_blank')">` : ''}
    ...
    function editRecord(id) {
      ...
      document.getElementById('f-type')._refreshCustomLabel?.();
      updateRecordSectionField();
      if (r.section) document.getElementById('f-section').value = r.section;
      ...
      pageItems.forEach(r => {
        html += `
          <tr>
            ...
            <td>${r.date}</td>
            <td><span class="tag">${r.plantation_type}</span>${getRecordSection(r) ? ` <span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${getRecordSection(r)}</span>` : ""}</td>
    ...
    function renderBusiness() {
      ...
      renderSectionsForContainer('business-sections', 'business');
    ...
    function computeHarvestShares(plantationType) {
      ...
      const gross = effectiveGrossSalesFor(plantationType);
    ...
    function renderShareTable(elId, plantationType) {
      ...
      const ownerPct = round2((1 - pct) * 100);
      const ownerShare = round2(sales - pool);
      ...
      if (gross === 0) {
        out.innerHTML = `<div class="empty-state"><span class="glyph">🌽</span>No Gross Sales yet for ${plantationType}. This fills in automatically once you log a sale on this plantation's page, or you can add revenue manually in the Reports tab.</div>`;
        return;
      }
      if (sales <= 0) {
      ...
      out.innerHTML = `
        ...
        <div class="totals-strip">
          ...
          <div class="t">Your share (${ownerPct.toFixed(0)}%)<b>${peso(ownerShare)}</b></div>
    ...
    function renderShares() {
      ...
      const maizeOwnerPct = round2((1 - HARVEST_SHARE_PCT["Maize Production"]) * 100);
      const beansOwnerPct = round2((1 - HARVEST_SHARE_PCT["String Beans Plantation"]) * 100);
      ...
      document.getElementById('share-summary-cards').innerHTML = `
        <div class="plantation-total-card">
          ...
          <div class="p-meta">Workers ${(HARVEST_SHARE_PCT["Maize Production"] * 100).toFixed(0)}% / You ${maizeOwnerPct.toFixed(0)}% of ${peso(maize.sales)} Net Sales (Gross ${peso(maize.gross)}) · ${maize.shares.length} worker${maize.shares.length !== 1 ? 's' : ''}</div>
          <div class="p-profit pos">Your share: <strong>${peso(round2(maize.sales - maize.pool))}</strong></div>
        </div>
        <div class="plantation-total-card">
          <div class="p-name">String Beans Plantation</div>
          <div class="p-total">${peso(beans.pool)}</div>
          <div class="p-meta">Workers ${(HARVEST_SHARE_PCT["String Beans Plantation"] * 100).toFixed(0)}% / You ${beansOwnerPct.toFixed(0)}% of ${peso(beans.sales)} Net Sales (Gross ${peso(beans.gross)}) · ${beans.shares.length} worker${beans.shares.length !== 1 ? 's' : ''}</div>
          <div class="p-profit pos">Your share: <strong>${peso(round2(beans.sales - beans.pool))}</strong></div>
        </div>
      `;
    ...
    function renderNetSalesInputs() {
      ...
      out.innerHTML = PLANTATION_TYPES.map(type => {
        ...
        const cs = computeSalesTotalsFor(type);
        const effective = effectiveGrossSalesFor(type);
        return `
        <div class="field">
          <label for="ns-${type}">${type} — Additional Gross Sales (₱) <span class="tag">manual, optional</span></label>
          <input type="number" min="0" step="0.01" class="ns-input" id="ns-${type}" data-type="${type}" value="${grossSales[type] ?? 0}">
          <div class="net-sales-readout">${cs.count > 0 ? `🛒 Auto from ${cs.count} logged customer sale(s): <b>${peso(cs.total)}</b> (${cs.quantity} kg) — added automatically, no action needed` : 'No customer sales logged yet for this plantation — use "Sell Produce" on its detail page.'}</div>
          <div class="net-sales-readout">Total Gross Sales: <b>${peso(effective)}</b> <span style="color:var(--faint)">(manual ${peso(grossSales[type] || 0)} + auto ${peso(cs.total)})</span></div>
          <div class="net-sales-readout">Net Sales: <b class="${ns < 0 ? 'neg' : ''}">${peso(ns)}</b> <span style="color:var(--faint)">(Total Gross − Expenditure ${peso(expenditureFor(type))})</span></div>
        </div>
      `;
      }).join('');
    }
    ...
    function renderPlantationTotals() {
      ...
      out.innerHTML = PLANTATION_TYPES.map(type => {
        ...
        const gross = effectiveGrossSalesFor(type);
    ...
    function renderCustomerSalesReport() {
      const out = document.getElementById('report-customer-sales');
      if (!out) return;
      const byType = {};
      plantationSales.forEach(s => {
        if (!byType[s.plantation_type]) byType[s.plantation_type] = { total: 0, quantity: 0, count: 0 };
        byType[s.plantation_type].total += s.total;
        byType[s.plantation_type].quantity += s.quantity;
        byType[s.plantation_type].count += 1;
      });
      const entries = Object.entries(byType).sort((a, b) => b[1].total - a[1].total);
      if (entries.length === 0) {
        out.innerHTML = `<div class="empty-state"><span class="glyph">🛒</span>No customer sales logged yet — record one from a plantation's detail page.</div>`;
        return;
      }
      const max = Math.max(...entries.map(([, v]) => v.total));
      out.innerHTML = entries.map(([type, v]) => `
        <div class="bar-row">
          <div class="label" title="${type}">${plantationEmoji(type)} ${type}</div>
          <div class="bar-track"><div class="bar-fill" style="width:${(v.total / max * 100).toFixed(1)}%"></div></div>
          <div class="amt">${peso(v.total)}</div>
        </div>
        <div style="margin:-4px 0 10px 0; padding-left:2px; font-size:11px; color:var(--faint);">${v.count} sale(s) · ${round2(v.quantity)} kg sold</div>
      `).join('');
    }
    ...
    function renderReports() {
      ...
      const avg = records.length ? total / records.length : 0;
      const totalGross = PLANTATION_TYPES.reduce((s, t) => s + effectiveGrossSalesFor(t), 0);
      const totalNetSales = PLANTATION_TYPES.reduce((s, t) => s + netSalesFor(t), 0);
      const totalYield = records.reduce((s, r) => s + (r.yield_kg || 0), 0);
      const totalCustomerSales = round2(plantationSales.reduce((s, x) => s + x.total, 0));
      const totalCustomerQty = round2(plantationSales.reduce((s, x) => s + x.quantity, 0));
      ...
      document.getElementById('stat-cards').innerHTML = `
        ...
        <div class="stat-card good"><div class="label">🛒 Total Customer Sales</div><div class="value">${peso(totalCustomerSales)}</div><div class="sub">${plantationSales.length} sale(s) · ${totalCustomerQty} kg sold</div></div>
      `;

      renderCustomerSalesReport();
      renderSectionsForContainer('report-sections', 'reports');
    ...
    function computeAlerts() {
      ...
      PLANTATION_TYPES.forEach(type => {
        ...
        const ns = netSalesFor(type);
        if (effectiveGrossSalesFor(type) > 0 && ns < 0) {
    ...
    function exportPDFReport() {
      ...
      PLANTATION_TYPES.forEach(type => {
        ...
        if (spent === 0 && effectiveGrossSalesFor(type) === 0) return;
        const ns = netSalesFor(type);
        doc.text(`${type}: expenditure ${peso(spent)} · gross sales ${peso(effectiveGrossSalesFor(type))} · net sales ${peso(ns)}`, 14, y);
        y += 6;
    ...
    async function saveSale() {
      ...
      const sections = getSectionsFor(selectedPlantationType);
      const sectionEl = document.getElementById('sale-section');
      const section = sections ? (sectionEl.value || sections[0]) : null;
      ...
      if (editingSaleId) {
        ...
        if (idx !== -1) {
          plantationSales[idx] = { ...plantationSales[idx], customer_name: customer, location, cellphone, section, quantity, price_per_unit: price, total, date };
        ...
        const sale = {
          ...
          customer_name: customer, location, cellphone, section,
      ...
      if (success) {
        ...
        renderSectionBreakdown(selectedPlantationType);
      }
    }
    ...
    function editSale(id) {
      ...
      if (s.section) document.getElementById('sale-section').value = s.section;
    ...
    function renderPlantationSales(type) {
      ...
      const hasSections = !!getSectionsFor(type);
      ...
      out.innerHTML = `<div class="table-wrapper"><table>
        <thead><tr><th>ID</th><th>Date</th>${hasSections ? '<th>Section</th>' : ''}<th>Customer</th><th>Location</th><th>Cellphone</th><th class="num">Quantity</th><th class="num">Current Price</th><th class="num">Total Cost</th><th></th></tr></thead>
        <tbody>
          ${sales.map(s => `
            <tr>
              <td class="num">#${s.id}</td>
              <td>${s.date}</td>
              ${hasSections ? `<td>${getSaleSection(s) ? `<span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${getSaleSection(s)}</span>` : '—'}</td>` : ''}
    ...
    function showPlantationDetail(type) {
      ...
      cancelEditSale();
      updateSaleSectionField();
      renderPlantationDetail(type);
      renderPlantationSales(type);
      renderSectionBreakdown(type);
      navigateTo('plantation-detail');
    }

    function renderSectionBreakdown(type) {
      const card = document.getElementById('pd-sections-card');
      const sections = getSectionsFor(type);
      if (!sections) { card.style.display = 'none'; return; }
      card.style.display = '';
      document.getElementById('pd-sections-label').textContent = `(${sections.length} sections)`;

      const grid = document.getElementById('pd-sections-grid');
      grid.innerHTML = sections.map(sec => {
        const st = computeSectionStats(type, sec);
        return `
          <div class="plantation-total-card">
            <div class="p-name">📐 ${sec}</div>
            <div class="p-total">${peso(st.expenditure)}</div>
            <div class="p-meta">${st.recordCount} record(s) · ${st.workerCount} worker log(s) · ${st.itemCount} item(s) bought · yield ${st.yieldKg} kg</div>
            <div class="p-meta">🛒 ${st.saleCount} sale(s) · ${st.salesQty} kg sold</div>
            <div class="p-profit ${st.net >= 0 ? 'pos' : 'neg'}">Sales ${peso(st.salesTotal)} − Expenditure ${peso(st.expenditure)} = <strong>${peso(st.net)}</strong></div>
          </div>
        `;
      }).join('');
    }

    function renderSectionsForContainer(containerId, mode) {
      const container = document.getElementById(containerId);
      const cardId = mode === 'business' ? 'business-sections-card' : 'report-sections-card';
      const card = document.getElementById(cardId);
      if (!container || !card) return;

      const typesWithSections = Object.keys(PLANTATION_SECTIONS).filter(t => records.some(r => r.plantation_type === t) || plantationSales.some(s => s.plantation_type === t));
      if (typesWithSections.length === 0) { card.style.display = 'none'; return; }
      card.style.display = '';

      container.innerHTML = typesWithSections.map(type => {
        const sections = getSectionsFor(type);
        const rows = sections.map(sec => computeSectionStats(type, sec));
        if (mode === 'business') {
          return `
            <div style="margin-bottom:18px;">
              <div style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--blue-bright);margin-bottom:8px;">${plantationEmoji(type)} ${type}</div>
              <div class="grid cols-3">
                ${rows.map(st => `
                  <div class="plantation-total-card">
                    <div class="p-name">📐 ${st.section}</div>
                    <div class="p-total">${peso(st.expenditure)}</div>
                    <div class="p-meta">${st.recordCount} record(s) · ${st.itemCount} item(s) bought · labor ${peso(st.laborCost)} + items ${peso(st.itemsCost)}</div>
                  </div>
                `).join('')}
              </div>
            </div>
        ...
        return `
          <div style="margin-bottom:10px;font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--blue-bright);">${plantationEmoji(type)} ${type}</div>
          <div class="table-wrapper"><table>
            <thead><tr><th>Section</th><th class="num">Records</th><th class="num">Expenditure</th><th class="num">Yield</th><th class="num">Sales</th><th class="num">Net</th></tr></thead>
            <tbody>
              ${rows.map(st => `
                <tr>
                  <td>${st.section}</td>
                  <td class="num">${st.recordCount}</td>
                  <td class="num">${peso(st.expenditure)}</td>
                  <td class="num">${st.yieldKg} kg</td>
                  <td class="num">${peso(st.salesTotal)}</td>
                  <td class="num">${peso(st.net)}</td>
                </tr>
              `).join('')}
            </tbody>
          </table></div>
    ...
    function renderPlantationDetail(type) {
      ...
      const sharesGrossSales = effectiveGrossSalesFor(type);
      ...
      document.getElementById('pd-formula').innerHTML =
        ...
        (sharesGrossSales > 0 && sharesGrossSales !== totalGrossIncome ? `<div style="margin-top:6px;color:var(--faint);">Note: the Harvest Shares / Reports figure for this plantation is ${peso(sharesGrossSales)} (customer sales logged here, plus any manual entry) — this page instead shows Gross Income from each harvest's recorded Yield × Price per kg.</div>` : '');
      ...
      if (recs.length === 0) {
        ...
        tableHtml = `<div class="table-wrapper"><table>
          <thead><tr><th>ID</th><th>Date</th>${hasSections ? '<th>Section</th>' : ''}<th>Yield</th><th class="num">Gross Income</th><th class="num">Workers</th><th class="num">Expenditure</th></tr></thead>
          <tbody>`;
        [...recs].sort((a,b) => b.date.localeCompare(a.date)).forEach(r => {
          tableHtml += `<tr><td class="num">#${r.id}</td><td>${r.date}</td>${hasSections ? `<td>${getRecordSection(r) ? `<span class="tag" style="color:var(--fuchsia-bright);border-color:var(--fuchsia-deep);">${getRecordSection(r)}</span>` : '—'}</td>` : ''}<td>${r.yield_kg ? r.yield_kg + ' kg' : '—'}</td><td class="num">${r.revenue ? peso(r.revenue) : '—'}</td><td class="num">${r.workers.length}</td><td class="num">${peso(r.total_expenditure)}</td></tr>`;
        });
    ...
    function refreshAll() {
      ...
      if (activeView === 'plantation-detail' && selectedPlantationType) { renderPlantationDetail(selectedPlantationType); renderPlantationSales(selectedPlantationType); renderSectionBreakdown(selectedPlantationType); }
    ...
    function buildGlobalSearchIndex(query) {
      ...
      records.forEach(r => {
        ...
        const haystack = `${r.plantation_type} ${r.section || ''} ${r.date} ${workerNames} ${jobDescs} ${itemNames}`.toLowerCase();
        if (haystack.includes(q)) {
          results.push({
            group: 'Business record', icon: '📋',
            label: `#${r.id} · ${r.plantation_type}${getRecordSection(r) ? ' · ' + getRecordSection(r) : ""} · ${r.date}`,
    ...
    document.getElementById('f-type').addEventListener('change', updateRecordSectionField);
    updateRecordSectionField();
    document.getElementById('f-date').value = todayISO();
    document.getElementById('p-date').value = todayISO();
    ...
    addItemRow();
    addPersonalItemRow();
    setupActivityImageHandlers();

    // Safety net: never let the loading spinner hang forever, no matter what fails below.
    const loadingSafetyTimer = setTimeout(() => {
      setLoading(false);
      console.warn('Loading took too long — forcing the overlay closed.');
    }, 8000);
    ...
    openDB().then(async () => {
      try {
        ...
        console.error('One or more data stores failed to load:', e);
        showToast('⚠️ Some data failed to load — showing what is available.', 'error', 5000);
      } finally {
